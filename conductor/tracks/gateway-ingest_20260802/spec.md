...
Este é o **primeiro serviço TypeScript** do monorepo (até aqui só Python via `uv`). O track estabelece também o toolchain Node (pnpm, Dockerfile Node) e a esteira CI para imagem não-Python.
...
- **Stack:** Node 22 + TypeScript, **Fastify**, **Zod** (validação in/out), **kafkajs** (producer). `pnpm` como package manager. App em `apps/gateway/` — primeiro membro não-`uv` do monorepo.
...
- **CI:** Gitea Actions ganha job de build da imagem Node (Dockerfile multi-stage pnpm), push pro registry interno, write-back de `tag=SHA` em `apps/gateway/chart/values-image.yaml`.
