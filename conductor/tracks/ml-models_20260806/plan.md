...
**Achados ao iniciar a Fase 5 (cluster local no ar pela primeira vez nesta track):**

1. **Schema drift nos marts** — os `Job`s de treino falhavam porque `daily_anomaly_features`/`first_touch_duration` no ClickHouse estavam desatualizados em relação ao SQL atual dos marts (`avg_opened_hour`, `opened_by`, `has_parent_incident`, `status` ausentes). Causa: os commits que adicionaram essas colunas (`48e5527`, `306ca38` — as próprias tasks 1.x/2.x desta track) nunca vieram acompanhados de um rerun do `data-pipeline` workflow. Corrigido rodando `argo submit --from workflowtemplate/data-pipeline -n data --watch` (documentado em `docs/data-pipeline.md`) — não foi preciso mudar código.
2. **Bug real de fuso horário em `split.py`** (`apps/ml-volume-model/src/split.py` e `apps/ml-breach-model/src/split.py`, duplicado byte-a-byte por design — ver nota da Fase 2) — `pd.to_datetime` em cima de uma coluna `DateTime64(UTC)` do ClickHouse retorna tz-aware; os limites do split (`train_end`/`validation_end`/`holdout_end`) são `pd.Timestamp` naive. A comparação `dates <= train_end_ts` estourava `TypeError: Invalid comparison between dtype=datetime64[ns, UTC] and Timestamp` — só aparecia no breach model porque `first_touch_duration.opened_at` é `DateTime64`, enquanto o volume model usa `daily_anomaly_features.date` (`Date`, sem tz). **Corrigido**: tz é descartado logo após o `pd.to_datetime` (`dates.dt.tz_localize(None)` quando `dates.dt.tz is not None`) nas duas cópias, com teste de regressão novo (`test_temporal_split_handles_tz_aware_dates`) em `tests/test_split.py` das duas apps. 17 + 12 testes verdes.
3. **Dataset real no cluster é uma amostra, não o histórico completo** — `default.daily_anomaly_features`/`first_touch_duration` no ClickHouse local têm só 606 incidentes (até 2025-01-04), contra as 122.543 linhas (até 2025-12-31) de `assets/incidents.csv`. `scripts/incident_producer.py` nunca rodou até o fim nesta instância do cluster. Sem ingestão completa, as partições de validação/holdout do split ficam vazias para os limites fixos (`train_end=2025-09-30` etc.) — o guard de `split.py` pega isso e falha alto, como projetado. **Não resolvido nesta sessão** — decisão explícita do usuário foi corrigir só o bug de tz agora; ingestão completa (rodar o producer contra o gateway, ~5-20min dependendo da latência) e o rerun dos `Job`s ficam pendentes antes de a task 5.1 poder concluir.
