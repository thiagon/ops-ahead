...
def temporal_split(
    ...
    Identical to ml-volume-model's src/split.py — every model in this track
    shares the same temporal boundaries and the same leakage-avoidance rule,
    but each training job ships as its own container image, so the function is
    duplicated rather than imported across apps.
    """
...
def _raise_if_any_partition_empty(
    ...
    downstream goes quietly wrong (NaN AUC-PR, a LightGBM fit on zero rows).
