...
| Monitoramento de drift (Evidently AI → métrica Prometheus) | Implementado — `apps/ml-trainer/src/drift/`, `evidently.Report`/`ValueDrift` (PSI + KS), novo domínio `drift_monitoring` em `trigger.ml`, métricas `ml_trainer_feature_drift_psi`/`_ks_pvalue` | **Revertido da Fase 1**: o desvio consciente registrado ali foi recusado pelo usuário em 2026-08-17, que também confirmou usar a biblioteca Evidently de verdade (não uma reimplementação manual de PSI/KS). Implementado na Fase 10 entre a janela de treino do modelo `Production` (lida do param `train_end` do run promovido) e a janela atual dos marts. |
